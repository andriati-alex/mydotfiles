...
local function factory(args)
    ...
    mpd_notification_preset = { title = "Now playing", timeout = 6, border_width = 2, border_color = "#d54466"}
                ...
                if notify == "on" and mpd_now.title ~= helpers.get_map("current mpd track") then
                    ...
                    if not string.match(mpd_now.file, "http.*://") then -- local file instead of http stream
                        ...
                        helpers.async({ shell, "-c", cover }, function(current_icon)
                            ...
                            mpd.id = naughty.notify({preset = common, shape = gears.shape.rounded_rect}).id
                        end)
                    else
                        mpd.id = naughty.notify({preset = common, shape = gears.shape.rounded_rect}).id
                    end
